...
async function updateActions() {
  ...
  actions.forEach((item) => {
    ...
    $('#appActionsDropdown .dropdown-menu a:last').before(newAction);
  });
}
...
$(document).ready(() => {
  ...
  $('.custom-action-scene.default button').click(e => {
    e.preventDefault();
    const selectedMethod = customActionModal.find('input[name="custom-action-method"]:checked').val();
  ...
  $('.custom-action-scene.prompt button').click(async e => {
    ...
    const menuTitle = customActionModal.find('#menuTitle').val().trim();
    const selectedMethod = customActionModal.find('input[name="custom-action-method"]:checked').val();
